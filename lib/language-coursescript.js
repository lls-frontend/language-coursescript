...
const courses = ['DARWIN', 'LT', 'PT', 'OCC', 'BE', 'PILOT', 'PHONICS', 'BELL', 'TOURISM', 'LINGOCHAMP', 'KION', 'KION EXAM', 'DARWIN BUSINESS', 'TELIS']
...
export default {
  ...
  config: {
    ...
    previewCourse: {
      type: 'object',
      properties: courses.reduce((res, item) => ({
        ...res,
        [item]: {
          title: item,
          type: 'boolean',
  ...
  insertCoreCourseID() {
    const id = '2' + (new Date()).getTime().toString(16).padStart(23, '0')
    const editor = atom.workspace.getActivePaneItem()
    editor.insertText(id)
  },

  insertBellID() {
    const id = (new Date()).getTime().toString(16).padStart(24, '0')
    const editor = atom.workspace.getActivePaneItem()
    editor.insertText(id)
  },
